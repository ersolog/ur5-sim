...
import numpy as np
from utils.mujoco_custom_viewer import MujocoMinimalViewer
"""
...
from utils.slider import MultiSliderClass
...
class MuJoCoParserClass(object):
    ...
    def get_xyzs(self):
        obj_names = self.get_body_names(prefix='obj_')
        n_obj = len(obj_names)
        xyzs = np.zeros((n_obj, 3))
        for b in range(n_obj):
            body_name = obj_names[b]
            jntadr  = self.model.body(body_name).jntadr[0]
            qposadr = self.model.jnt_qposadr[jntadr]
    ...
    def get_body_name_closest(self,xyz,body_names=None,verbose=False):
        ...
        for body_idx,body_name in enumerate(body_names):
            p_body = self.get_p_body(body_name=body_name)
            print(f"pbody for {body_name}: {p_body}\n")
        ...
        print(f"cloesset: {body_name_closest}\n")
